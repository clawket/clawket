...
## Purpose
7 skill 은 `skills/<name>/SKILL.md` + `skills/<name>/RULE.md` 두 파일 단위로 구성된다 (총 14 파일). tarball 부분 추출 / 부분 release / 누락된 commit 으로 한 파일이라도 빠지면 `/clawket-plan-design`, `/clawket-verify-batch`, `/clawket-verify-loop` 등 진입점이 runtime 에 resolve 실패한다. `ensureInstalled` 의 fast-path 가 이 14 파일 존재를 강제로 검사해 "healthy" 오판을 막아야 한다.
...
- 새 skill 추가 시 `verifySkills` 의 array 갱신 누락 → 새 skill 누락이 silent

## Evidence
- `adapters/shared/claude-hooks.cjs:1119-1144` — `verifySkills(pluginRoot)` 본체 (warn-only, 누락 파일마다 stderr + `ok = false`)
- `adapters/shared/claude-hooks.cjs:1126-1134` — hardcoded skill list: `['clawket-dashboard', 'clawket-plan-design', 'clawket-scenario-author', 'clawket-verify-batch', 'clawket-verify-loop', 'clawket-scenario-refine', 'clawket-defect-fix']`
- `adapters/shared/claude-hooks.cjs:1185` — fast-path 가 `skillsOk` 를 `cliOk/daemonOk/webOk` 와 동급 AND 조건으로 사용
- `tests/skills-integrity.test.cjs` — 7 skill × 2 file 존재 검증 + 모든 RULE.md 가 `상태: STABLE — Clawket plugin 정본` 라벨 보유 강제
...
## Enforcement gap
- `verifySkills` 가 `ok=false` 일 때 fast-path 에서 `skillsOk` 가 false 가 되어 재설치 트리거 — 하지만 `runSetup` 자체는 skills/ 를 재배포하지 않는다 (binary + web bundle 만)
- `plugin.json#skillsList` 와 `verifySkills` 의 hardcoded array 가 분리되어 있어 한쪽만 변경 가능 (현재 manual sync)
...
- skill 명을 rename 시 `tests/skills-integrity.test.cjs` 의 `SKILLS` 배열도 동시 변경
...
### DON'T
- `verifySkills` 를 warn-only 로 다시 약화시키지 마라 (현재 fast-path 가 `skillsOk` 를 hard AND 로 사용) — 약화하면 partial install 이 다시 silent 통과
