# 시나리오 정련 규칙 (Scenario Refinement)

> **상태: STABLE — Clawket plugin 정본.** `/clawket-scenario-refine` skill 의 규칙
> 본체. `/clawket-scenario-author` 의 작성 규칙을 공유하며 본 파일에서는 특히
> **갱신 규칙** 과 **라운드 내 정련** 섹션을 운영 인터페이스로 사용한다.
> plan-driven workflow 의 수렴 루프와 통합. atomic 분해는 결과물(작성 시점에
> 끝내는 것)이 아니라 과정(루프 안에서 진행되는 것).

사용자 시나리오의 라운드 내 정련 절차를 정의한다. 시나리오는 의도,
plan design 은 실행 명세화, 검증 플로우는 검증 — 세 룰은 단일 수렴 루프 안에서
통합 운영된다.
...
## /clawket-scenario-refine skill 인터페이스

본 룰은 Clawket plugin 정본 skill `skills/clawket-scenario-refine/` 의 RULE 본체. skill 은
다음 입력으로 호출된다:
