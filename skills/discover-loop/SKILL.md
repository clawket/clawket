---
name: discover-loop
description: 발견-수렴 루프 메인 엔진 — Round R sub-agent dispatch + TSV evidence emit + bulk sync transcription + 3-way 수렴 판정 + 다음 라운드 schedule. PDD-cycle skill 대체. Clawket plugin 정본 skill (v3.0). RULE.md (pdd.md + qa-flow.md v3.0) 적용.
---

# /discover-loop — 발견-수렴 루프 메인 엔진

PDD v3.0 + qa-flow v3.0 의 통합 운영 인터페이스. 9000-태스크 자율 런 회고 데이터로
distill 한 발견-수렴 루프 본체. `/scenario-author` (Phase 0) 와 `/pdd-plan` (Phase 1)
산출물을 입력으로 받아 Round R 사이클을 운영한다.
...
각 Unit 마다 sub-agent 1개 dispatch. 1 agent ≤ 30 시나리오 (9000-런 회고: 87/agent
는 attention dilution 발생).
...
### 3. TSV evidence schema (v3.0 — 7 필드)
...
필드 강제 (R2 DOGFOOD-013/017/043/044 distill):
...
- `evidence`: `<file>:<line>` 또는 reasoning 요약. **모든 status 에 필수**
  (DOGFOOD-043 — pass 도 회귀 비교 기준선 보존을 위해 file:line 인용).
- `tier_used`: agent tier (`haiku|sonnet|opus`) — 비용 추적용. 항상 채움.
- `batch_id`: `BATCH-<26-char-ULID>` — 동일 batch 묶음 추적용 (DOGFOOD-044
  — 옵셔널 폐기, 모든 row 에 필수).
- `escalation_reason`: `tier_used=opus` 일 때 필수 (qa-flow §2.5). sonnet/haiku
  에선 선택. 6 필드 legacy TSV 도 backward-compat 으로 받지만 verify-tsv 가
  schema_version 경고를 emit.
...
**권장 경로 (9000-런 검증)**: `clawket discover-loop sync <tsv> --unit U
--cycle C` 한 번 호출 — 데몬이 status 매핑 (pass→done / defect→blocked /
scenario_error→cancelled) + scenario_amendment 추출 (DOGFOOD-016) +
escalation_reason persistence (DOGFOOD-017) + retry queue knowledge (DOGFOOD-025)
+ Round R evidence knowledge (DOGFOOD-018/019) 를 한 트랜잭션 안에서 처리.
...
이외의 분기를 가지지 않는다 (R2 distill).

### 5. 3-way 수렴 판정

라운드 종료 시 `clawket discover-loop status --plan PLAN-...` 호출. 데몬은
**`tasks.qa_status` OR `tasks.status` fallback** 으로 카운트한다 (R2
DOGFOOD-039 fix). 두 컬럼이 lock-step 이라 어느 컬럼이 부분 누락돼도 수치는
일관.
...
- **scenario_error ≥ 1** → `/scenario-refine` 호출 자동 트리거. 처리 절차:
  1. cancelled QA task body 의 `scenario_amendment` 필드 (DOGFOOD-016 으로
     persisted) 를 읽는다.
...
등록** 후 사용자 confirm 요청. 9000-런 §8 회고 — 단조 감소가 정상 패턴, 증가
는 결함 수정이 새 결함을 만든 신호.

### 6. 다음 라운드 schedule

수렴 미도달 시 `clawket discover-loop next-round --previous-plan PLAN-R` 호출.
데몬은 (R2 DOGFOOD-004 fix):
1. previous plan 의 active cycle 들을 자동 complete.
2. ALREADY_CONVERGED 가드 (DOGFOOD-033) — converged plan 에 next-round 호출 시
   400 거부.
...
for unit_info in manifest.units:
  for batch_id in unit_info.batch_ids:
    spawn_subagent(
      ...
      output_format="TSV (7 fields, R2 schema)",
    )
```
...
batch=<batch_id>"` TSV 7번째 컬럼에 박는다 (DOGFOOD-017 strict).

9000-런 회고: 87/agent 배치는 Δ ≈ 0.20 관찰. 30/agent 는 Δ < 0.05 안정. 초과
시 같은 batch 의 후반만 재 dispatch (다른 sub-agent + Opus).

### C. /scenario-refine 자동 dispatch

`status` 응답의 `scenario_error_count > 0` 시 즉시 `/scenario-refine` 슬래시
호출. cancelled QA task 의 `scenario_amendment` (DOGFOOD-016) 를 입력으로
받아 atomic 분해 / 의도 재정의 / 삭제 결정. 결정 사유는:
...
데몬은 plan.description 갱신 API 만 노출 (DOGFOOD-006 자동 채움 외엔 agent 가
판단).
...
## Anti-pattern 거부 (PDD v3.0 + qa-flow v3.0 동기화)
...
- 2.x 런타임 상태 수정 (`~/.local/share/clawket/`, `~/.cache/clawket/`,
...
### 라운드 종료 시
- [ ] 모든 batch 의 TSV evidence row 가 schema 검증 통과 (7 필드 v3.0)
- [ ] **모든 status** 의 row 에 evidence (file:line) 가 있다 (R2 DOGFOOD-043)
- [ ] tier_used=opus row 는 escalation_reason 채워짐 (R2 DOGFOOD-017)
- [ ] batch_id 가 모든 row 에 채워짐 (R2 DOGFOOD-044)
...
- **신규 핵심 가설**: 시나리오·코드 공진화 — 양 layer 동시 수렴
- **신규 메커니즘**: Sub-agent batch + TSV evidence + bulk sync 분리
- **신규 강제**: scenario_id NOT NULL + evidence file:line 필수 (defect/scenario_error)
- **구 PDD A4 (Cycle ⊂ Unit) 기각** — 9000-런 schema 가 강제한 적 없음
- **구 PDD O4 (cycle 첫 task = scope) 기각** — 자연 해소
- **구 X7~X9 (cross-unit cycle 등) 기각** — 운영 판단 영역으로 환원
