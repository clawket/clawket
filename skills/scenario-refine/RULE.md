# 시나리오 작성 규칙 (User Scenario Authoring, v3.0)

> **참조**: 본 파일은 `skills/scenario-author/RULE.md` 와 동일한 scenario-authoring
> v3.0 룰 본체다. `/scenario-refine` skill 은 특히 **§갱신 규칙** 과 **§라운드
> 내 정련** 섹션을 운영 인터페이스로 사용한다.

> **상태: STABLE — Clawket plugin 정본.** PDD `skills/pdd/RULE.md` v3.0 발견-수렴
> 루프와 통합. 9000-태스크 자율 런 회고로 distill. 가장 큰 변화: **atomic 분해를
> 결과물(작성 시점에 끝내는 것)이 아니라 과정(루프 안에서 진행되는 것)으로
> 재정의.** 시나리오는 사전-*완전* 이 아니라 사전-*예비* + 라운드 내 정련.
...
### v3.0 추가 가설: 시나리오는 mutable

시나리오는 사전-*완전* 이 아니다. **사전-예비 (큰 그림 + atomic 시나리오 초안)**
+ **라운드 내 정련 (scenario_error 처리)** 의 두 단계로 구성된다. 9000-런의
R2-R3 에서 459 시나리오가 reclassify 된 것은 시나리오층 정련의 정상 패턴.
시나리오 mutability 의 갱신 사유는 **"의도 부적절"** 에 한정 (qa-flow.md §3
규칙 #6 참조).
...
## 입자 규칙 (Atomicity, v3.0 — 결과물이 아니라 과정)

**1 시나리오 = 1 testable assertion.** 다만 v3.0 에선 atomic 분해를 *작성
시점에 끝내야 하는 결과물* 이 아니라 *발견-수렴 루프 안에서 도달하는 과정*
으로 본다.
...
- 9000-런 데이터: R1 1218 시나리오 → R3 에서 459 reclassify (390 atomic 분해,
  나머지 의도 재정의/삭제) → R4 이후 안정
...
## 갱신 규칙 (중요, v3.0 — /scenario-refine 인터페이스)
...
2. 3-way 분기:
   ...
   - **삭제**: ID 영구 비움 (재사용 금지). cancelled QA task comment 에 "이
     ID 는 V3 에서 삭제됨" 흔적.
...
- ✅ 가능: "이 시나리오의 expected result 가 v3 제품 비전과 어긋남 — 의도
  재정의"
- ✅ 가능: "이 기능은 v4-scope 으로 deferred — 시나리오 삭제, 새 knowledge 로 이관"
...
## /scenario-author skill 인터페이스 (v3.0)
...
- 9000-런 데이터: 시나리오는 R2-R3 에서 정련, 코드는 R4-R7 에서 수렴 — 두
  layer 가 시간차로 안정화되는 게 자연 패턴
