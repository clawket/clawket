...
> **상태: STABLE — Clawket plugin 정본.** 본 룰은 9000-태스크 자율 런 (Clawket
> v3 surface plan, 2026-04~05) 의 데이터 회고로 distill 된 v2 → v3.0 갱신본이다.
> 검증 통과 항목은 살리고 (A1, A3, A5, A6), 데이터로 기각된 항목은 삭제하며
> (구 A4 Cycle⊂Unit, 구 O4 cycle-첫-task), 구 룰엔 없었지만 9000 런이 실제로
> 작동한 메커니즘 (sub-agent dispatch, /loop, TSV evidence, batch reasoning)
> 을 정식 메커니즘으로 추가했다. Clawket plugin 정본 skill 로 승급 완료
> (`skills/pdd/RULE.md` 가 정본).

솔로 프로덕트 오너(자기 제품을 자기가 만드는 1인 컨텍스트) 의 개발 방법론.
`scenario-authoring.md` / `qa-flow.md` 와 한 쌍 — 시나리오는 의도, QA 는 검증,
PDD 는 그 사이의 **실행 명세화** 를 담당한다. v3.0 부터 셋은 단일 **발견-수렴
루프 (Discovery-Convergence Loop)** 로 통합 운영된다.
...
### v3.0 핵심 가설 (시나리오·코드 공진화)

**시나리오와 코드는 공진화한다.** 시나리오는 사전-*완전* 이 아니라 사전-*예비*.
발견-수렴 루프 안에서 시나리오는 atomic 까지 분해되고, 코드는 시나리오를
만족할 때까지 회귀한다. **두 layer 가 동시에 0 으로 수렴**할 때 종료. 이
가설은 9000-런의 R2-R3 시나리오층 안정화 + R4-R7 코드층 수렴이라는 자연
패턴으로 입증되었다 (구 룰은 이걸 "예외 처리" 라 불렀지만 실제로는 주
메커니즘).

## 두 레이어 분리 (PDD 의 본질, v3.0 갱신)
...
**의도 레이어 (v2 → v3 변화)**: 구 룰은 "처음부터 완전 명세, 700~2000 시나리오
사전에 다 박는다" 였다. 9000-런 회고: R2-R3 에서 459 시나리오가 부적절로
reclassify 됐다 — 사전 *완전* 은 false precision. 갱신: **사전 예비 설계
(큰 그림 + 도메인별 atomic 시나리오 초안)** + **라운드 내 정련 (scenario_error
처리로 atomic 분해 / 의도 재정의 / 삭제)**. 시나리오는 mutable 이지만 갱신
사유는 "의도 부적절" 에 한정 (qa-flow.md §scenario_error 참조).

**실행 레이어**: Cycle 활성화 시점에 Cycle 의 task 를 batch 로 생성. 9000-런
에선 sub-agent N 명이 1 unit 씩 분담해 시나리오를 reasoning 하고 TSV emit →
Python 드라이버가 16-병렬 bulk sync 로 task DB 에 transcribe 했다. **batch
생성은 anti-pattern 아니다** — 시나리오 1:1 task 매핑이 보장되면.

## 4 레이어 모델 (Clawket 매핑, v3.0)
...
Cycle ──── 1 라운드 (실행 단계, sub-agent dispatch 단위)
   │       — Cycle 은 Unit 1개에 묶이지 않아도 됨 (cross-unit 허용, v3 변화)
   │       — 라운드 단위 진행: Round R cycle 1개 + Round R+1 cycle 1개...
   │       — 동시 활성 cycle 가능 (병렬 라운드)
   │
   ▼
Task ───── 실행 단위 (Cycle 안에서 batch 생성)
           — 1 Task ↔ 1 시나리오 (또는 sub-spec) 환원 (A5)
           — scenario_id + evidence 필드 채워짐 (강제)
```

## Axioms (불변 공리, v3.0)

- **A1. Plan/Unit 은 spec 이다, idea 가 아니다.** (살림)
  모든 항목은 외부 관찰자가 동일하게 충족 여부를 판단 가능해야 한다.
- **A2. 의도는 사전 예비 설계 + 라운드 내 정련.** (수정 — 구 *완전 설계* 기각)
  큰 그림과 atomic 시나리오 초안은 사전. scenario_error 발견 시 라운드 안에서
  amend / delete (사유: 의도 부적절). 갱신은 audit knowledge + cancelled task
  comment 양쪽에 영구 기록.
- **A3. 실행은 단계적 생성 + sub-agent batch.** (살림 + 강화)
  Cycle 활성화 시점에 그 cycle 의 task 를 batch 생성. sub-agent N 명이 1 unit
  씩 분담 reasoning. 미리 모든 라운드 task 박지 않는다.
- **A4. 트레이서블 단방향 위계.** (구 A5 → 새 A4)
  모든 Task → 정확히 1 시나리오(또는 sub-spec) 환원. 환원 불가 = 스펙 외, 거부.
  v3.0 부터 `tasks.scenario_id` 컬럼으로 schema 강제.
- **A5. Red-Green-Refactor 매핑.** (구 A6 → 새 A5)
  - PDD Red = 라운드 R 의 미충족 시나리오 (cycle 활성 직후)
  - PDD Green = 라운드 R 통과 + 수렴 조건 충족
  - PDD Refactor = 시나리오 정련 (scenario_error → atomic 분해 / 의도 재정의 / 삭제)
- **A6. 진실 공급원 단일.** (구 A7 → 새 A6)
  Clawket Plan/Unit/Cycle/Task = 진실. 보조 문서 (knowledge, plans/*.md) = 미러.
- **A7. Tool-first, Automation-second.** (구 A8 → 새 A7)
  Plan 의 자동화 항목은 수동 도구 신뢰성 입증 후에만 포함.
- **A8. Sub-agent batch reasoning + bulk sync transcription.** (신규)
  9000-런이 작동한 핵심 메커니즘. reasoning 은 sub-agent 가 batch 단위 (≤ 30
  시나리오/agent) 로 수행, sync 는 Python 드라이버가 TSV → DB transcription
  으로 분리. 두 단계는 명확히 구분되며 sync 단계엔 evidence 가 동반되어야 한다.

### v3.0 에서 기각된 구 axioms

- **구 A4 (Cycle ⊂ Unit)**: **기각.** 9000-런의 R4 fix wave 는 cross-unit
  cycle (daemon=10, cli=6, clawket=1) 로 자연스럽게 작동했다. `cycles.unit_id`
  스키마 강제도 v3.0 schema 에 미반영 (실측 결과). cross-unit cycle 은
  허용된다.
- **구 O4 (cycle 첫 task = scope + 나머지 task 생성)**: **기각.** 9000-런 내내
  단 한 번도 따른 적 없음. sub-agent batch 가 메타-회귀를 이미 해결하므로 룰
  자체가 불필요.

## 발견-수렴 루프 (Discovery-Convergence Loop, v3.0 신규)
...
1. **1 agent / 1 unit / ≤ 30 시나리오** (강제). 9000-런의 87 시나리오/agent
   배치는 attention 분산 위험으로 관찰됐다 (R4 large unit 결과 신뢰도 저하
   가능성). 30 이하가 reasoning 품질 보장 임계.
...
3. **Bulk sync ≠ reasoning**. Python ThreadPoolExecutor 16-worker 등의
   드라이버는 TSV → DB transcription 만 수행. reasoning 결정을 sync 단계에
   끼워넣지 않는다. sync 코드는 TSV 의 status 를 그대로 옮겨야 한다.
...
- **X3**. **시나리오 환원 불가 Task (scenario_id NULL)** → 스펙 외, 거부.
  v3.0 부터 `task-created` hook 으로 scenario_id 패턴 검증.
- **X4**. Done 정의 = "구현 완료" 같은 자기참조 → 외부 검증 명제로 재정의
- **X5**. 수렴 조건 부재 → 재분해
- **X6**. 시간 기반 종료 조건 ("2주 후") → 산출물 기반 재정의
- **X7**. **Reasoning batch size > 30 / agent** (신규) → A8 위반.
  attention 분산 위험. 분할 dispatch 강제.
- **X8**. **TSV evidence 부재 sub-agent 산출** (신규) → A8 위반.
  reasoning 흔적이 영속화 안 됨. 재실행.
- **X9**. **Bulk sync 안에 reasoning 결정 끼워넣기** (신규) → A8 위반.
  sync 코드는 TSV → DB 매핑만. status 결정은 agent reasoning 산출이어야 함.

### v3.0 에서 기각된 구 anti-patterns

- **구 X7 (cross-unit cycle)**: **기각**. 9000-런 R4 fix wave 가 cross-unit 으로
  자연스러웠다. cycle 은 unit 에 묶이지 않는다.
- **구 X8 (동일 unit 동시 cycle)**: **기각**. 직렬/병렬은 운영 판단이지 룰 강제
  대상 아니다.
- **구 X9 (cycle 활성 전 task 미리 박기)**: **기각**. sub-agent batch 생성이
  구 X9 의 우려를 자연 해소.

## Operational Rules (Clawket 통합, v3.0)
...
- **O8**. **2.x 런타임 불가침** (신규, v3.0 dev 단계 한정).
  배포된 2.x 플러그인의 binary / daemon / cache / DB 는 절대 수정 / 삭제 금지.
  ALTER TABLE ADD COLUMN 같은 비파괴 가산만 허용. DROP / DELETE / TRUNCATE 절대
  금지. git reset / commit / push / tag / release 절대 금지 (사용자 명시 지시
  시에만).

### v3.0 에서 기각된 구 operational rules

- **구 O4 (cycle 첫 task = scope + 나머지 task 생성)**: **기각**. 메타-cycle
  무한 회귀는 sub-agent batch 가 이미 해결.
...
## 실험 → 정본 승격 기준 (EXPERIMENTAL → STABLE, v3.0)

본 룰이 다음을 통과하면 Clawket plugin 정본 스킬로 승급한다:

- [ ] 1 프로젝트 이상에서 1개 Plan 전체 lifecycle 완주 (발견-수렴 루프 1 회
      완주) — 9000-런 (v3 surface plan, 2026-04~05) 으로 partial 통과 (단,
      reasoning batch size 87 위반)
...
- PDD v3.0 은 솔로 PO 컨텍스트의 방법론. 9000-런 회고로 distill.
- 시나리오 700~2000 개를 사전에 박는 것이 정상이지만, 라운드 안에서 정련된다.
  R2-R3 의 459 시나리오 reclassify 는 정상 패턴.
- Cycle 수가 Unit 별로 다른 것이 정상 (Unit A = 1 cycle, Unit B = 4 cycle).
  의도가 가변이 아니라, 실행 라운드가 가변.
- Sub-agent reasoning batch size 30 은 9000-런의 87/agent 데이터로부터 도출된
  보수적 임계. 향후 데이터로 보정.
- v2 → v3 의 가장 큰 변화는 두 가지:
  (1) 시나리오층의 mutability 인정 (구 A2 의 *완전* 가설 기각)
  (2) sub-agent batch + bulk sync 분리의 명시적 정식화 (구 룰엔 한 줄도 없었음)
- 본 룰은 STABLE — Clawket plugin 정본. 정식 hook 강제는 v3.0 부터 본격 도입
  (PreToolUse / SubagentStart / PostToolUse 매처 조합) 이며, 스키마 컬럼
  (`scenario_id`, `evidence`, `batch_id`) 도 v3.0
  schema 작업 (U2) 진행 중.
